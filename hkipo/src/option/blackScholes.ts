export class blackScholes {
    ...
    getPremium(right: string, S: number, X: number, t: number, r: number, v: number) {
        ...
        var d1 = (Math.log(S / X) + (r + Math.pow(v, 2) / 2) * t) / (v * Math.sqrt(t));
    ...
    getImpliedVolatility(right: any, S: any, X: any, t: any, r: any, P: number) {
        ...
        var test = 0;
        var volatility = 0.0001;
        var upper = 999.99;
        var lower = 0;
        var range = Math.abs(lower - upper);
    ...
    getTheta(right: string, S: number, X: number, t: number, r: number, v: number) {
        ...
        if (right.toUpperCase().indexOf('C') !== -1) {
            ...
            return (-(S * (Math.exp(-(Math.pow(d1, 2)) / 2) / Math.sqrt(2 * Math.PI)) * v) / (2 * Math.sqrt(t)) - r * X * Math.exp(-(r * t)) * this.getNormDist(d2)) / 365;
        }
        else if (right.toUpperCase().indexOf('P') !== -1) {
